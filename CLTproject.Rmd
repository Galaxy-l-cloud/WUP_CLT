...
```
For the sample size of 9, we see that the mean has remained similar or in close numerical range to the other sample sizes. According to the CLT the sd has been reduced by a little less than half. In this graph we see two prominent peaks.
...
The CLT states that the sd for a sample size of 16 has changed by less than 5 integers. The mean again has remained similar to the other sample sizes. In this graph we see only two prominent peaks.
...
Here we see the graph is rounding out, there is one prominent peak, there is a vertex as the graph becomes a parabolic shape. The CLT states that the sd this time of sample size 25 has changed by around 5 integers. The mean still remains close to the other means of the other sample sizes.
